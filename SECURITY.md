...
workflows on the AX41 and rpi pools; reducing its CVE surface and keeping it
fresh is the point of the project.
...
Please include the affected tag (`:latest` or `:YYYYMMDD`) and digest if known,
the issue and its impact, and reproduction steps where possible.
...
- Hardening of the *workflows* that run on the runner, or of the host Docker
  daemon the runner talks to — those are the operator's responsibility (see
  `~/.setup`).
...
| Tag | Supported |
|---|---|
| `:latest` | ✅ rebuilt weekly |
| `:YYYYMMDD` | ✅ immutable snapshots; repull `:latest` for fixes |
